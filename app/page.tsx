...
export default function LandingPage() {
  useEffect(() => {
    const autoRedirect = async () => {
      try {
        const response = await fetch("/api/random-link")
        const data = await response.json()

        if (data.url) {
          // Try multiple redirect methods simultaneously for iOS
          window.location.replace(data.url) // Most reliable for iOS
  ...
  const handleRandomLinkClick = async () => {
    try {
      const response = await fetch("/api/random-link")
      const data = await response.json()
    ...
    <main className="min-h-screen bg-[#F3EEE8] flex items-center justify-center p-4">
      <div className="w-full max-w-[680px] mx-auto py-12">
        ...
        <div className="px-4 space-y-3">
          ...
          >
            <span className="text-center">{profileConfig.mainLink.title}</span>
          </Button>
        </div>
